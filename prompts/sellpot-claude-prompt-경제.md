...
| 글자수 | 처리 |
|---|---|
| **0~59자** | ❌ 너무 짧음. 옆 장면과 합치기 |
| **60~136자** | ✅ OK (이상적: 70~120자) |
| **137자 이상** | ❌ 17초 초과. 의미 단위(문장 경계)로 분할해서 각 조각이 60~136자 들어오게 |

### 묶음 만드는 절차 (반드시 이 순서)
1. 한 문단(빈 줄로 구분된 단위)부터 시작
2. 같은 시각적 순간/주제면 묶을 후보 → 글자수 카운트
3. **70~120자 들어오면** 그 묶음 확정 (보통 3~5문장)
4. **120자 넘는데 아직 시각 흐름 같으면**: 136자까지는 OK. 그 이상이면 분할
5. **70자 미만이면** 다음 문장도 묶을지 검토 (단, 시각 전환 있으면 끊고 미니 장면이라도 옆과 합칠 후보로 표시)
6. 묶음 확정 직전 **반드시 글자수 다시 카운트** → 137자 이상이면 즉시 분할

### 응집도 룰
- 한 장면 = 하나의 시각적 순간/주제
- 같은 인물·장소·시점이면 글자수 한도까지 묶음
- 다른 인물·장소·시점으로 넘어가면 묶음 끊기 (글자수 부족해도 끊기)
- **분할 시**: 같은 시각적 흐름 안에서 나뉘면 nano_prompt는 카메라 각도/구도/포인트만 변주

### 타입별 글자수 캡
- **ai**: ≤ 136자 (위 절대 룰)
- **real**: ≤ 136자 (동일)
- **claude_design**: ≤ 200자 (인포그래픽은 한 화면에 더 많이 담아도 됨)

### ❌ 자주 하는 실수
- 1~2문장씩 잘게 쪼개기 → 30~50자 미니 장면 폭증 (60자 미만 금지)
- 7~8문장 한 묶음으로 → 200자 넘는 장면 발생 (137자 이상 금지)
- 글자수 카운트 안 하고 "이 정도면 되겠지" 추측 → **반드시 카운트**

### 예시 (실제 카운트해보기)
- "트럼프가 발표했다. 시장은 흔들렸다. 다우는 3% 떨어졌다." (35자) → ❌ 60자 미만, 다음 문장도 합치기
- "트럼프가 발표하자 시장은 즉각 반응했어요. 다우 3%, 나스닥 5%, S&P 4% 동시 폭락. 한 시간 만에 시총 2조 달러가 증발했죠." (95자) → ✅ OK
- "[200자짜리 긴 묶음]" → ❌ 137자 이상. 의미 단위로 잘라 두 장면(예: 100자 + 100자)으로
