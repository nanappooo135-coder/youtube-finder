...
|---|---|---|---|
| **real** (실사) | **≥ 15%** | 15~25% | 최소 15개, 권장 15~25개 |
| **claude_design** (인포그래픽) | **≥ 10%** | 10~15% | 최소 10개, 권장 10~15개 |
| **ai** (일러스트) | 나머지 | 60~75% | 60~75개 |

**❌ 위반 사례 (실제 발생함, 절대 반복 금지)**:
- 100장면 중 real 5개(5%) → **부족, real 후보를 더 발굴해서 15개 이상으로 재분류**
- 100장면 중 claude_design 5개(5%) → **부족, 데이터·비교·구조 언급되는 장면을 더 찾아서 10개 이상으로 재분류**

**검증 절차 (출력 직전 반드시 수행)**:
1. 전체 scenes 배열에서 type별 개수 카운트
2. real 비율 = real개수 / 전체 × 100 → **15 미만이면 재작업**
3. claude_design 비율 = claude_design개수 / 전체 × 100 → **10 미만이면 재작업**
4. 부족하면: AI로 분류했던 장면 중 아래 후보를 real/claude_design으로 전환:
   - **real 후보**: 실존 인물 이름(트럼프/시진핑/머스크/CEO명)·실제 기관(IMF/연준/삼성)·실제 장소(월스트리트/실리콘밸리)·실제 사건(2008 금융위기/911) 언급 → real로
   - **claude_design 후보**: GDP/환율/실업률/주가/무역 통계·국가 비교·시장 점유율·공급망 구조·타임라인·N단계 프로세스 언급 → claude_design으로

claude_design은 경제 데이터·그래프·국가 비교·무역 흐름도·시장 구조도 등에 적극 활용.
real은 실존 인물(정치인, CEO, 경제학자), 실제 기관, 역사적 장소가 자주 등장.
...
1. **ai** — AI 일러스트(나노바나나2). 일반 장면, 도시·건물·군중·상황 묘사·비유. 기본값이지만 **real/claude_design 후보를 먼저 골라낸 뒤 남은 것**으로 배정.
2. **real** — 실사(구글 이미지). 실존 인물(이름 명시), 실제 책 《》, 유명 기관·장소·건물·로고. **최소 15%, 권장 15~25%**.
